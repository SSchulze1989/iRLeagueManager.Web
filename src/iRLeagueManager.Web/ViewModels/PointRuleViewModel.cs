...
public sealed class PointRuleViewModel : LeagueViewModelBase<PointRuleViewModel, PointRuleModel>
{
    ...
    {
        get => BonusPoints.Select(x => new BonusPointConfig(x.Type, x.Name, (int)x.Value, (int)x.Points, x.Conditions));
        set => BonusPoints = value.Select(x => new BonusPointModel() { Type = x.OptionId, Name = x.Name, Value = x.Position, Points = x.Points, Conditions = x.Conditions }).ToList();
    }
